...
function MyAdsCard({item ,userId}) {
...
const handleDeleteBtn =()=>{
    ...
    dispatch(ADS_DELETE_ASYNC({id:_id , userId:userId , card_Id:product._id} ))
}

    return (
        <div className={cls['myAdsCard']}>
                ...
                {Upload.map(el => {
                    if (el.thumbUrl === "") {
                        return <Image
                           ...
                           key={el.thumbUrl} src='error' width='100%'/>
                    }
                    return (
                        <Image key={el.thumbUrl} src={el.thumbUrl}/>
                    )
                })}
            </div>
            <div className={cls['myAdsCard-content']}>
